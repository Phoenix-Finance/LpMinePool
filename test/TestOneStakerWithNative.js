...
contract('MinePoolProxy', function (accounts){
    ...
    before("init", async()=>{
      ...
      //set mine coin info                       0xe2ee960E3BC3d5C651A8528dBDf09DB31051F12B
        let res = await proxy.setPoolMineAddress("0x0000000000000000000000000000000000000000",fnxToken.address);
   ...
   it("[0010] stake test and check mined balance,should pass", async()=>{
      ...
      res = await proxy.stake(0,"0x0",{from:staker1,value:web3.utils.toWei("1",'ether')});
      time1 = await tokenFactory.getBlockTime();
      console.log(time1.toString(10));

      //check totalStaked function
      let totalStaked = await proxy.totalStaked();
      assert.equal(web3.utils.fromWei(totalStaked),1);
  ...
  it("[0030] stake out,should pass", async()=>{
    console.log("\n\n");
    let preLpBlance = await  web3.eth.getBalance(staker1);
    ...
    let afterLpBlance = await web3.eth.getBalance(staker1);

    let lpdiff = web3.utils.fromWei(afterLpBlance) - web3.utils.fromWei(preLpBlance);
    console.log("afterLpBlance diff=" + lpdiff);
    //because gas consume,the eth unstake is smaller than input
    assert.equal((lpdiff<diff)&&(lpdiff+0.1>diff),true);
  })


  it("[0050] get back left mining token,should pass", async()=>{
    console.log("\n\n");
    await web3.eth.sendTransaction({from:accounts[5],to:proxy.address,value:web3.utils.toWei("1",'ether')});
    ...
    let preRecieverEthBalance = await web3.eth.getBalance(staker1);
 //   console.log("before mine balance = " + preRecieverEthBalance);
    ...
    let afterRecieverEthBalance = await web3.eth.getBalance(staker1);
    console.log("after mine balance = " + afterRecieverEthBalance);

    let ethdiff = web3.utils.fromWei(afterRecieverEthBalance) - web3.utils.fromWei(preRecieverEthBalance);
    assert.equal(ethdiff,1);

  })

  it("[0060] get back left mining token,should pass", async()=>{
